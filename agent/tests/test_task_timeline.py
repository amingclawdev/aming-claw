    ...
    return [
        {
            ...
            "status": "accepted",
            "payload": {
                "schema_version": "observer_hotfix_pre_reason.v1",
                "reason": "normal worker route is blocked",
                "allowed_files": ["agent/governance/task_timeline.py"],
                "blocked_normal_route_evidence": ["close gate needs contract policy"],
                "dirty_scope_before_mutation": {"dirty_files": []},
                "planned_verification": ["pytest"],
                "forbidden_actions": ["fabricate worker evidence"],
            },
        },
        {
            "id": 1002,
            ...
            "payload": {
                ...
                "pre_reason_event_id": 1001,
                "what_changed": "contract-aware close gate projection",
                "changed_files": ["agent/governance/task_timeline.py"],
                "verification_evidence_refs": {"pytest": "passed"},
                "deviations_from_plan": [],
                "remaining_close_gate_evidence": ["qa", "close_ready"],
            },
        },
    ]
...
class TestTaskTimeline(unittest.TestCase):
    ...
    def test_root_route_context_close_action_card_requests_close_scoped_token(self):
        from agent.governance import observer_session, server
        ...
        self.assertEqual(action_card["schema_version"], "observer_root_route_close_action_card.v1")
        self.assertEqual(action_card["status"], "refresh_required")
        self.assertEqual(action_card["classification"], "expected_action_token_refresh")
        self.assertTrue(action_card["expected_behavior"])
        self.assertFalse(action_card["is_system_bug"])
        self.assertTrue(action_card["do_not_waive"])
        self.assertEqual(
            action_card["current_route_token_allowed_actions"],
            ["task_timeline_append"],
        )
        self.assertEqual(
            action_card["issue_route_token_request"]["allowed_actions"],
            ["backlog_close"],
        )
        self.assertEqual(
            action_card["issue_route_token_request"]["parent_route_token_ref"],
            issued["route_token_ref"],
        )
        self.assertIn(
            "qa_review",
            {item.get("event_kind") for item in action_card["evidence_refs"]},
        )
        self.assertIn(
        ...
        self.assertIn(
            ...
            action_card["issue_route_token_request"]["target_files"],
        )
        self.assertEqual(result["next_legal_action"]["id"], "issue_close_scoped_route_token_ref")
        self.assertEqual(result["next_legal_action"]["allowed_action"], "backlog_close")
    ...
    def test_root_route_context_close_action_card_uses_close_token_ref_when_ready(self):
        ...
        action_card = result["close_gate_projection"]["action_card"]
        self.assertEqual(action_card["status"], "ready")
        self.assertEqual(action_card["next_action"], "backlog_close")
        self.assertEqual(action_card["route_token_ref"], issued["route_token_ref"])
    ...
    def test_root_route_context_close_action_card_reports_done_for_fixed_row(self):
        ...
        bug_id = "BUG-ROOT-CONTEXT-CLOSE-ACTION-FIXED"
        issued = self._issue_route_token(
            bug_id,
            allowed_actions=["backlog_close"],
        )
        ...
        self.assertTrue(result["close_gate_projection"]["can_close"])
        action_card = result["close_gate_projection"]["action_card"]
        self.assertEqual(action_card["status"], "done")
        self.assertTrue(action_card["terminal"])
        self.assertEqual(action_card["terminal_status"], "FIXED")
        self.assertEqual(action_card["classification"], "already_fixed")
        self.assertEqual(action_card["action"], "done")
        self.assertNotEqual(action_card.get("next_action"), "backlog_close")
        self.assertFalse(action_card["route_token_refresh_required"])
        self.assertNotIn("issue_route_token_request", action_card)
        self.assertEqual(result["next_legal_action"]["id"], "done")
        self.assertEqual(result["next_legal_action"]["action"], "done")
        self.assertNotEqual(result["next_legal_action"].get("allowed_action"), "backlog_close")
        self.assertNotIn('"route_token":', json.dumps(result, sort_keys=True))
        self.assertNotIn("session_token", json.dumps(result, sort_keys=True))
    ...
    def test_route_context_artifact_ref_named_cleanup_does_not_supersede_event(self):
        from agent.governance import task_timeline

        events = _route_context_consumption_events()
        events[0]["id"] = 6499
        events[0]["artifact_refs"] = {"route_identity_cleanup": "timeline:6498"}

        gate = task_timeline.mf_route_context_gate_verification(events)

        self.assertFalse(gate["route_identity_cleanup"]["applied"])
        self.assertIn("route_context", gate["present_requirement_ids"])
        self.assertNotIn("route_context", gate["missing_requirement_ids"])
    ...
    def test_independent_qa_must_follow_latest_implementation(self):
        from agent.governance import task_timeline
        ...
        self.assertFalse(blocked["independent_qa_gate"]["passed"], blocked)
        self.assertEqual(
    ...
    def test_route_identity_cleanup_scopes_read_receipt_gate_to_current_lineage(self):
        ...
        current_qa["id"] = 2047
        fresh_current_qa = _route_context_qa_verification_event()
        fresh_current_qa["id"] = 2052
        cleanup = {
            "id": 2048,
    ...
    def test_observer_hotfix_close_policy_replaces_worker_but_still_requires_qa(self):
        from agent.governance import task_timeline
        ...
        self.assertFalse(blocked["passed"], blocked)
        self.assertTrue(blocked["route_context_gate"]["passed"], blocked)
        self.assertEqual(blocked["route_context_gate"]["missing_requirement_ids"], [])
        self.assertEqual(
            blocked["route_context_gate"]["not_applicable_requirement_ids"],
            [
                "bounded_implementation_worker_dispatch",
                "mf_subagent_startup",
            ],
        ...
        self.assertEqual(
            blocked["route_context_gate"]["replaced_requirement_ids"],
            ["independent_verification_lane"],
        )
        self.assertFalse(blocked["independent_qa_gate"]["passed"], blocked)
        self.assertEqual(
        ...
        self.assertEqual(groups["bounded_worker"]["missing"], [])
        self.assertEqual(groups["independent_verification"]["missing"], [])
        self.assertEqual(groups["independent_qa"]["missing"], ["independent_qa"])
        self.assertEqual(
            groups["contract_policy"]["not_applicable_by_contract"],
            [
                "bounded_implementation_worker_dispatch",
                "mf_subagent_startup",
            ],
        ...
        reminder_actions = {
            action["id"] for action in blocked["route_context_reminder"]["next_actions"]
        }
        self.assertIn("honor_contract_close_policy", reminder_actions)
        self.assertNotIn("dispatch_bounded_worker", reminder_actions)
        self.assertNotIn("start_worker", reminder_actions)

    def test_observer_hotfix_close_policy_allows_close_after_independent_qa(self):
        from agent.governance import task_timeline
        ...
        self.assertEqual(
            ready["route_context_gate"]["contract_close_gate_policy"]["template_id"],
            "observer_hotfix_direct_mutation.v1",
        )
        self.assertTrue(
            ready["route_context_gate"]["checks"][
                "bounded_worker_not_applicable_by_contract"
            ]
        )
    ...
    def test_observer_direct_exception_accepts_qa_commands_run_test_evidence(self):
        ...
        events = [
            ...
            },
            {
                ...
                "payload": {
                    ...
                    "tests_run": [
                        {
                            "cmd": "python3 -m pytest agent/tests/test_task_timeline.py",
                            "status": "passed",
                        }
                    ],
                },
            ...
            {
                "id": 7113,
                "event_type": "mf.observer_direct_verification",
                "event_kind": "verification",
                "phase": "verification",
                "status": "passed",
                "actor": "qa:observer-direct",
        ...
        blocked_events = copy.deepcopy(events)
        blocked_events[2]["payload"]["commands_run"] = [
            {"cmd": "git diff --check", "status": "passed"},
        ]
        blocked = task_timeline.mf_close_gate_verification(
            blocked_events,
        ...
        blocked_direct_gate = blocked["observer_direct_close_exception_gate"]
        self.assertFalse(blocked_direct_gate["passed"], blocked)
        self.assertIn(
            "tests_or_test_results",
            blocked_direct_gate["missing_requirement_ids"],
        )
        ...
        self.assertNotIn("tests_or_test_results", direct_gate["missing_requirement_ids"])
        self.assertEqual(direct_gate["verification_event"]["id"], 7113)
    ...
    def test_observer_direct_exception_rejects_changed_files_outside_scope(self):
        ...
        direct_gate = blocked["observer_direct_close_exception_gate"]
        self.assertFalse(direct_gate["passed"], blocked)
        self.assertIn(
            "changed_files_within_allowed_scope",
            direct_gate["missing_requirement_ids"],
        )
        self.assertEqual(
            direct_gate["changed_file_scope"]["unexpected_changed_files"],
            ["agent/governance/server.py"],
        )
    ...
    def test_observer_direct_exception_requires_allowed_changed_file_scope(self):
        ...
        self.assertFalse(direct_gate["changed_file_scope"]["enforced"])
    ...
    def test_observer_hotfix_contract_policy_requires_qa_without_parallel_topology(self):
        ...
        self.assertFalse(blocked["passed"], blocked)
        self.assertTrue(blocked["independent_qa_gate"]["required"], blocked)
        self.assertTrue(blocked["independent_qa_gate"]["contract_policy_required"])
        self.assertFalse(blocked["independent_qa_gate"]["topology_required"])
        self.assertEqual(
            blocked["independent_qa_gate"]["missing_requirement_ids"],
            ["independent_qa"],
        )
    ...
    def test_observer_hotfix_successor_policy_overrides_onboard_root_template(self):
        ...
        self.assertEqual(
            route_gate["contract_close_gate_policy"]["explicit_template_id"],
            "onboard_contract.v1",
        )
        self.assertEqual(
            route_gate["contract_close_gate_policy"]["successor_template_id"],
            "observer_hotfix_direct_mutation.v1",
        )
        self.assertEqual(
            route_gate["contract_close_gate_policy"]["template_id"],
            "observer_hotfix_direct_mutation.v1",
        )
        self.assertTrue(
    ...
    def test_observer_hotfix_successor_policy_waits_for_hotfix_evidence(self):
        ...
        self.assertFalse(blocked["passed"], blocked)
        self.assertEqual(
            blocked["route_context_gate"]["contract_close_gate_policy"][
    ...
    def test_mf_parallel_close_gate_accepts_qa_canonical_verification_child_route(self):
        from agent.governance import task_timeline

        bug_id = "BUG-QA-CANONICAL-VERIFICATION-ROW-SCOPED"
        contract = {
            "template_id": "mf_parallel.v1",
            "contract_instance_id": bug_id,
            "route_topology_policy": {
        ...
        parent_identity = {
            "route_id": "route-qa-canonical-parent",
            "route_context_hash": "sha256:qa-canonical-parent-route",
            "prompt_contract_id": "rprompt-qa-canonical-parent",
            "prompt_contract_hash": "sha256:qa-canonical-parent-contract",
        }
        child_identity = {
            "route_id": "route-qa-canonical-child",
            "route_context_hash": "sha256:qa-canonical-child-route",
            "prompt_contract_id": "rprompt-qa-canonical-child",
            "prompt_contract_hash": "sha256:qa-canonical-child-contract",
        }
        qa_event = {
            "id": 6579,
            "event_type": "mf.verification",
            ...
            "backlog_id": bug_id,
            "project_id": "aming-claw",
            "task_id": "cex-qa-canonical-verification",
            "payload": {
                "schema_version": "qa_canonical_verification.v1",
                "meta_contract_gate": {
                    "action": "qa_verification",
                    "role": "qa",
                    "status": "passed",
                ...
                "contract_evidence": {
                    "independent_verification": {"status": "passed"},
                    "focused_tests": ["agent/tests/test_task_timeline.py"],
                },
            },
            "verification": {
                "contract_evidence": [
                    ...
                    {"requirement_id": "focused_tests", "status": "passed"},
                ]
            },
        }
        _attach_server_action_scope_route_token_lineage(
            ...
            child_identity,
            parent_identity=parent_identity,
            route_token_ref="rtok-qa-canonical-verification",
            acceptance_source="server_route_token_action_scope",
        )
            ...
            [
                {
                    ...
                    "backlog_id": bug_id,
                    "project_id": "aming-claw",
                    "task_id": "cex-worker-qa-canonical",
                },
                {
                    "event_kind": "close_ready",
                    "phase": "close",
                ...
                *_route_context_consumption_events(parent_identity),
                qa_event,
            ],
            contract=contract,
        )
        ...
        accepted_action_events = {
            item["event"]["id"]
            for item in ready["route_context_gate"]["accepted_action_scope_lineages"]
        }
        self.assertIn(6579, accepted_action_events)

    def test_cross_ref_rejects_non_qa_generic_verification_spoofed_schema_child_route(self):
        from agent.governance import task_timeline

        bug_id = "BUG-NON-QA-GENERIC-VERIFICATION-CHILD-ROUTE"
        contract = {
            "template_id": "mf_parallel.v1",
            "contract_instance_id": bug_id,
            "route_topology_policy": {
        ...
        parent_identity = {
            "route_id": "route-non-qa-parent",
            "route_context_hash": "sha256:non-qa-parent-route",
            "prompt_contract_id": "rprompt-non-qa-parent",
            "prompt_contract_hash": "sha256:non-qa-parent-contract",
        }
        child_identity = {
            "route_id": "route-non-qa-child",
            "route_context_hash": "sha256:non-qa-child-route",
            "prompt_contract_id": "rprompt-non-qa-child",
            "prompt_contract_hash": "sha256:non-qa-child-contract",
        }
        verification = {
            "id": 6580,
            "event_type": "mf.verification",
            ...
            "backlog_id": bug_id,
            "project_id": "aming-claw",
            "task_id": "cex-generic-verification-child",
            "payload": {
                "schema_version": "qa_canonical_verification.v1",
                "meta_contract_gate": {
        ...
        _attach_server_action_scope_route_token_lineage(
            ...
            child_identity,
            parent_identity=parent_identity,
            route_token_ref="rtok-non-qa-generic-verification",
            acceptance_source="server_route_token_action_scope",
        )
        ...
        self.assertFalse(blocked["passed"], blocked)
        rejected = blocked["cross_ref_gate"]["rejected_cross_ref_evidence"]
        self.assertEqual([item["id"] for item in rejected], [6580])
        self.assertIn(
            "runtime_context_id",
            rejected[0]["route_token_child_lineage"]["missing_fields"],
        )

    def test_cross_ref_rejects_non_qa_generic_verification_spoofed_qa_meta_gate_child_route(self):
        from agent.governance import task_timeline

        bug_id = "BUG-NON-QA-GENERIC-VERIFICATION-QA-META-SPOOF"
        contract = {
            "template_id": "mf_parallel.v1",
            "contract_instance_id": bug_id,
            "route_topology_policy": {
        ...
        parent_identity = {
            "route_id": "route-non-qa-meta-parent",
            "route_context_hash": "sha256:non-qa-meta-parent-route",
            "prompt_contract_id": "rprompt-non-qa-meta-parent",
            "prompt_contract_hash": "sha256:non-qa-meta-parent-contract",
        }
        child_identity = {
            "route_id": "route-non-qa-meta-child",
            "route_context_hash": "sha256:non-qa-meta-child-route",
            "prompt_contract_id": "rprompt-non-qa-meta-child",
            "prompt_contract_hash": "sha256:non-qa-meta-child-contract",
        }
        verification = {
            "id": 6581,
            "event_type": "mf.verification",
            ...
            "backlog_id": bug_id,
            "project_id": "aming-claw",
            "task_id": "cex-generic-verification-meta-spoof",
            "payload": {
                "schema_version": "qa_canonical_verification.v1",
                "meta_contract_gate": {
                    "action": "qa_verification",
        ...
        _attach_server_action_scope_route_token_lineage(
            ...
            child_identity,
            parent_identity=parent_identity,
            route_token_ref="rtok-non-qa-generic-verification-meta-spoof",
            acceptance_source="server_route_token_action_scope",
        )
            ...
            [
                {
                    ...
                    "backlog_id": bug_id,
                    "project_id": "aming-claw",
                    "task_id": "cex-worker-non-qa-meta-spoof",
                },
                {
                    "event_kind": "close_ready",
                    "phase": "close",
        ...
        self.assertFalse(blocked["passed"], blocked)
        rejected = blocked["cross_ref_gate"]["rejected_cross_ref_evidence"]
        self.assertEqual([item["id"] for item in rejected], [6581])
        self.assertIn(
            "runtime_context_id",
            rejected[0]["route_token_child_lineage"]["missing_fields"],
        )
    ...
    def test_backlog_timeline_gate_repair_view_reuses_close_ready_close_token(self):
        from agent.governance import server, task_timeline

        bug_id = "BUG-MF-REPAIR-REUSE-CLOSE-READY-TOKEN"
        close_commit = "c10eed0000000000000000000000000000000000"
        issued = self._issue_route_token(
            bug_id,
            allowed_actions=["task_timeline_append", "backlog_close"],
        )
        server.handle_backlog_upsert(
            _ctx(
                path_params={"bug_id": bug_id},
                body={
                    "title": "MF timeline repair reuses close-ready token",
                    "status": "OPEN",
                    "mf_type": "observer_hotfix",
                    "force_admit": True,
                    "target_files": ["agent/governance/task_timeline.py"],
        ...
        result = server.handle_backlog_timeline_gate(
            _ctx(
                {"view": "repair", "close_commit": close_commit},
                path_params={"bug_id": bug_id},
            )
        )
        ...
        self.assertTrue(result["ok"])
        self.assertTrue(result["can_close"], result)
        hint = result["repair_summary"]["backlog_close_route_token_request_hint"]
        self.assertTrue(hint["backlog_close_authorized"])
        self.assertFalse(hint["authorization_required"])
        self.assertEqual(hint["action"], "backlog_close")
        self.assertEqual(hint["route_token_ref"], issued["route_token_ref"])
        self.assertEqual(
            result["repair_summary"]["deterministic_actions"][0],
            hint,
        )
        self.assertNotIn("issue_route_token_request", hint)

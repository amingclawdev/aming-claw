...
class TestDispatchExecutorOnly:
    """Executor-only deploy: POST to redeploy/executor + legacy restart_executor."""
    ...
    def test_executor_only_dispatches_both_paths(self):
        ...
        # Legacy path also called
        mock_legacy.assert_called_once()

    def test_executor_pre_success_write(self):
        """AC7: task marked SUCCEEDED with redeploy_pending BEFORE restart_executor."""
        patches = _make_patches()
        call_order = []

        def track_mark(*a, **kw):
            call_order.append("mark_succeeded")

        def track_restart(*a, **kw):
            call_order.append("restart_executor")
            return True

        with patch("agent.deploy_chain.restart_executor", side_effect=track_restart), \
             patches["rebuild_governance"], \
             patches["restart_gateway"], \
             patches["smoke_test"], \
             ...
             patches["post_manager_redeploy"], \
             patches["save_report"], \
             patch("agent.deploy_chain._mark_task_succeeded_pre_kill", side_effect=track_mark):
            run_deploy(
                ["agent/executor.py"],
                project_id="aming-claw",
                task_id="t1",
        ...
        assert call_order.index("mark_succeeded") < call_order.index("restart_executor")
...
class TestDispatchBothGovernanceAndExecutor:
    ...
    def test_both_dispatched(self):
        ...
        mock_mgr.assert_called_once()       # governance via manager

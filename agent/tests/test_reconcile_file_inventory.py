...
def test_inventory_uses_shared_language_policy_for_mixed_js_ts_fixture(tmp_path):
    ...
    assert rows_by_path["web/src/config.json"]["scan_status"] == "orphan"
    assert rows_by_path["web/src/config.json"]["effective_binding_status"] == "unbound"
    assert rows_by_path["web/vite.config.ts"]["file_kind"] == "config"
    assert rows_by_path["web/vite.config.ts"]["scan_status"] == "pending_decision"

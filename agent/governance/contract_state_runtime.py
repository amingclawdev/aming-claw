...
CLI_AGENT_MANAGED_PROFILE_TOOLING_PLUGIN_VERSION = (
    "0.2.1+codex.20260808153702"
)
CLI_AGENT_MANAGED_PROFILE_TOOLING_MCP_SERVER = "aming-claw"
CLI_AGENT_QA_BOOTSTRAP_GUIDE_PROMPT_TEMPLATE = (
    "$aming-claw:aming-claw-onboard\n"
    "Immediately use that skill to call managed MCP `onboard_route_guide` "
    "with exactly project_id={project_id}, backlog_id={backlog_id}, role=qa, "
    "and work_type=qa_verification. Do not guess or call a curl endpoint. "
    "Execute nothing else before these two onboarding instructions complete, "
    "including qa_session_register, graph_query, any ContractRuntime read, any "
    "test, or any final response.\n"
    "Then proceed as the ContractRuntime-authorized independent QA verifier.\n"
    ...
    "After onboarding, authoritative bounded QA order:\n"
    "3. In assigned_worktree, run exactly `git rev-parse HEAD` and use that "
...
_MF_PARALLEL_DEFAULT_REQUIREMENTS = [
    ...
    },
    {
        ...
        "detail": (
            "observer records canonical current-full graph reconcile evidence after "
            "merge lands and before close_ready"
        ),
        ...
        [
            ...
            },
            {
                ...
                "detail": (
                    ...
                    "target_project_root, graph_basis, canonical_base_snapshot_id, "
                    "base_commit_sha, candidate_commit_sha, server-derived changed_files, "
                    "candidate_diff_hash, and no missing trace ids or identity mismatches"
                ),
...
def _ticket_launch_identity(value: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    identity = {
        ...
        "worker_id": _ticket_public_text(source.get("worker_id")),
        "worker_slot_id": _ticket_public_text(source.get("worker_slot_id")),
        "observer_command_id": _ticket_public_text(
            source.get("observer_command_id")
        ),
...
def _ticket_authority_mismatches(
    ...
    comparisons = {
        ...
        "worker_slot_id": action.get("worker_slot_id"),
        "observer_command_id": action.get("observer_command_id"),

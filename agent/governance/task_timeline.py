...
def _route_parent_child_dispatch_identity(
    ...
    *,
    ...
    def _rejected(reason: str, **extra: Any) -> tuple[dict[str, str], dict[str, Any]]:
        return identity, {
    ...
    parent_lineage = _first_deep_mapping(value, "parent_route_lineage")
    child_lineage = _first_deep_mapping(value, "child_route_lineage")
    if not service_generated or source != "observer_runtime_text_prepare":
        return _rejected("not_service_generated_runtime_text_dispatch")
    if worker_role != "mf_sub":
        return _rejected("missing_or_invalid_worker_role", worker_role=worker_role)
    missing_fields = [
        field
        for field, token in {
            "runtime_context_id": runtime_context_id,
    ...
    if missing_fields:
        return _rejected("missing_dispatch_lineage_fields", missing_fields=missing_fields)
    if task_id == parent_task_id:
        return _rejected("task_id_matches_parent_task_id")
    if expected_parent and parent_task_id != expected_parent:
        return _rejected(
            "parent_task_id_mismatch",
            expected_parent_task_id=expected_parent,
            parent_task_id=parent_task_id,
        )
    if not parent_lineage:
        return _rejected("missing_parent_route_lineage")
    if not child_lineage:
        return _rejected("missing_child_route_lineage")

    def _lineage_value(lineage: Mapping[str, Any], field: str) -> str:
        return str(lineage.get(field) or "").strip()

    parent_mismatches = []
    for field, expected_value in parent_hint.items():
        actual_value = _lineage_value(parent_lineage, field)
    ...
    parent_route_id = _lineage_value(parent_lineage, "route_id")
    parent_visible_manifest = _lineage_value(
        parent_lineage,
        "visible_injection_manifest_hash",
    )
    parent_route_token_ref = _lineage_value(parent_lineage, "route_token_ref")
    parent_backlog_id = _lineage_value(parent_lineage, "backlog_id")
    if not parent_route_id:
        parent_mismatches.append({"field": "route_id", "expected": "<present>", "actual": ""})
    if not parent_visible_manifest:
        parent_mismatches.append({
            "field": "visible_injection_manifest_hash",
            "expected": "<present>",
            "actual": "",
        })
    if expected_parent and parent_backlog_id != expected_parent:
        parent_mismatches.append({
            "field": "backlog_id",
            "expected": expected_parent,
            "actual": parent_backlog_id,
        })
    if not parent_route_token_ref:
        parent_mismatches.append({
    ...
    if parent_mismatches:
        return _rejected(
            ...
            parent_route_lineage_mismatches=parent_mismatches,
        )

    child_mismatches = []
    child_expected = {
        "route_id": child_route_id,
        "route_context_hash": identity.get("route_context_hash", ""),
        "prompt_contract_id": identity.get("prompt_contract_id", ""),
        "prompt_contract_hash": identity.get("prompt_contract_hash", ""),
        "route_token_ref": route_token_ref,
    }
    for field, expected_value in child_expected.items():
        actual_value = _lineage_value(child_lineage, field)
    ...
    if not _lineage_value(child_lineage, "visible_injection_manifest_hash"):
        child_mismatches.append({
            "field": "visible_injection_manifest_hash",
            "expected": "<present>",
            "actual": "",
    ...
    if child_mismatches:
        return _rejected(
            "child_route_lineage_mismatch",
            child_route_lineage_mismatches=child_mismatches,
        )
    child_parent_ref = _lineage_value(child_lineage, "parent_route_token_ref")
    if not child_parent_ref:
        return _rejected("missing_route_token_ref_binding")
    if child_parent_ref != parent_route_token_ref:
        return _rejected(
            "route_token_ref_binding_mismatch",
            expected_parent_route_token_ref=parent_route_token_ref,
            actual_parent_route_token_ref=child_parent_ref,
        )
    return dict(parent_hint), {
        "schema_version": "bounded_worker_dispatch_child_route_lineage.v1",
        "accepted": True,
...
def mf_route_context_gate_verification(
    ...
    rejected_dispatch_lineages: list[dict[str, Any]] = []
    ...
    for index, raw_event in enumerate(rows):
        ...
        if "bounded_implementation_worker_dispatch" in categories:
            ...
            if lineage:
                ...
                if lineage.get("accepted"):
                    accepted_dispatch_lineages.append(lineage_event)
                else:
                    rejected_dispatch_lineages.append(lineage_event)

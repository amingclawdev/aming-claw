...
@dataclass(frozen=True)
class SafeRefPrestartupReissueAuthority:
    """Server-derived authority for one copy-safe pre-startup token rotation.
...
def _runtime_context_timeline_event_order_key(
    event: Mapping[str, Any],
) -> tuple[int, int, str, str, str]:
    """Return a deterministic append-only rank for timeline projections."""
    ...
    return (
        1 if match else 0,
        sequence,
        _runtime_context_text(event.get("created_at")),
        event_ref,
        json.dumps(event, sort_keys=True, separators=(",", ":"), default=str),
    )


def _runtime_context_event_lineage_identity(
    ...
    return {
        ...
        "implementation_event_ref": (
            ...
            or _runtime_context_deep_text(payload, "implementation_event_ref")
        ),
        "implementation_lineage_ref": (
            _runtime_context_text(payload.get("implementation_lineage_ref"))
            or _runtime_context_deep_text(payload, "implementation_lineage_ref")
        ),
    }


def _runtime_context_event_lineage_match_score(
    event: Mapping[str, Any],
    lineage: Mapping[str, Any],
) -> int:
    actual = _runtime_context_event_lineage_identity(event)
    score = 0
    expected_implementation_ref = _runtime_context_text(
    ...
    for field in (
        ...
        expected_value = _runtime_context_text(lineage.get(field))
        actual_value = _runtime_context_text(actual.get(field))
...
def _runtime_context_select_lineage_event(
    ...
    candidates = [
        ...
        for event in events
        if isinstance(event, Mapping)
        and _runtime_context_event_lineage_match_score(event, lineage) > 0
    ]
    if not candidates and not any(
        _runtime_context_text(lineage.get(field))
        for field in (
            "commit_sha",
            "implementation_event_ref",
            "implementation_lineage_ref",
    ...
    return (
        max(
            candidates,
            key=lambda event: (
                _runtime_context_event_lineage_match_score(event, lineage),
                _runtime_context_timeline_event_order_key(event),
            ),
        )
        if candidates
        else {}
...
def _runtime_context_timeline_derived_evidence(
    ...
    *,
    ...
    ordered_events.sort(key=_runtime_context_timeline_event_order_key)
    ...
    worker_commit_ref = ""
    worker_commit_sha = ""
    worker_commit_event: dict[str, Any] = {}
    verification_refs: list[str] = []
    verification_events: list[dict[str, Any]] = []
    finish_gate: dict[str, Any] = {}
    finish_gate_events: list[dict[str, Any]] = []
    ...
    for event in ordered_events:
        ...
        if event_kind == "worker_commit" and is_worker_lane_evidence:
            ...
            worker_commit_event = dict(event)
        ...
        if event_kind in {
            ...
            finish_gate_events.append(dict(event))
        if event_kind in {"close_ready", "close_request", "finish_request"}:
            timeline_refs["close_ready_event_ref"] = event_ref
            close_evidence = {
                "event_id": event_ref,
    ...
    worker_lineage = _runtime_context_event_lineage_identity(worker_commit_event)
    selected_implementation = _runtime_context_select_lineage_event(
        implementation_events,
        worker_lineage,
    )
    if selected_implementation:
        timeline_refs["latest_implementation_event_ref"] = (
            _runtime_context_event_ref(selected_implementation)
        )
        selected_implementation_identity = _runtime_context_event_lineage_identity(
            selected_implementation
        )
        worker_lineage = {
            **selected_implementation_identity,
            **{
                key: value
    ...
    if verification_refs:
        timeline_refs["historical_verification_event_refs"] = (
            _runtime_context_dedupe(verification_refs)
        )
        current_verification_refs = [
            ...
            if (
                ...
                or _runtime_context_event_lineage_match_score(
                    event,
    ...
    timeline_refs["finish_event_refs"] = _runtime_context_dedupe(
        [_runtime_context_event_ref(event) for event in finish_gate_events]
    )
    selected_finish = _runtime_context_select_lineage_event(
        finish_gate_events,
        worker_lineage,
    )
    if selected_finish:
        selected_finish_ref = _runtime_context_event_ref(selected_finish)
        timeline_refs["finish_event_ref"] = selected_finish_ref
        finish_gate = {
            "event_id": selected_finish_ref,
            "source_ref": selected_finish_ref,
            "checkpoint_id": _runtime_context_deep_text(
                selected_finish,
                "checkpoint_id",
            ),
            "payload": public_contract_revision_payload(
                selected_finish.get("payload") or {}
            ),
        }
    else:
        selected_attestation = _runtime_context_select_lineage_event(
            finish_attestation_events,
            worker_lineage,
        )
        if selected_attestation:
            selected_attestation_payload = _runtime_context_mapping(
    ...
    timeline_refs["finish_time_attestation_event_refs"] = _runtime_context_dedupe(
        [
            _runtime_context_event_ref(event)
    ...
    timeline_refs["route_action_precheck_event_refs"] = _runtime_context_dedupe(
        [
            _runtime_context_event_ref(event)
    ...
    route_candidates = [
        ...
        and (
            not timeline_refs.get("finish_event_ref")
            or not _runtime_context_deep_text(event, "worker_finish_gate_ref")
            or _runtime_context_deep_text(event, "worker_finish_gate_ref")
            == timeline_refs.get("finish_event_ref")
        )
    ]
    selected_route_precheck = (
        max(route_candidates, key=_runtime_context_timeline_event_order_key)
        if route_candidates
        else {}
    )
    if selected_route_precheck:
        timeline_refs["route_action_precheck_event_ref"] = (
            _runtime_context_event_ref(selected_route_precheck)
        )

    for selected_event in (
        selected_implementation,
        worker_commit_event,
        _runtime_context_select_lineage_event(
            finish_attestation_events,
            worker_lineage,
        ),
        selected_finish,
        ...
        if selected_event:
            _collect_route_identity(selected_event)
    return {
...
def build_safe_ref_prestartup_reissue_authority(
    context: BranchTaskRuntimeContext,
    *,
    contract_execution_id: str,
    read_receipt_ref: str,
    ...
    authority = SafeRefPrestartupReissueAuthority(
        ...
        worker_id=worker_id,
        worker_slot_id=worker_slot_id,
        agent_id=worker_id or worker_slot_id or context.agent_id,
        allocation_owner=str(
...
def reissue_mf_subagent_runtime_session_token(
    ...
    *,
    ...
    host_startup_id: str = "",
    host_session_id: str = "",
    safe_ref_authority: SafeRefPrestartupReissueAuthority | None = None,
    ttl_seconds: Any = None,
    now_iso: str = "",
) -> dict[str, Any]:
    """Rotate an mf_sub runtime session token for an active matching context.

    Raw-token callers retain the narrow expired-token renewal path.  A caller
    that no longer has raw auth may instead present the current copy-safe ref
    together with a server-derived, source-backed pre-startup authority.  The
    ref is never accepted as a general write credential.
    """
    ...
    presented_ref = str(session_token_ref or "").strip()
    raw_token_mode = bool(fence and presented_token)
    safe_ref_mode = bool(presented_ref and safe_ref_authority is not None)
    ...
    if raw_token_mode:
        ...
        authority = safe_ref_authority
        if not isinstance(authority, SafeRefPrestartupReissueAuthority):
            raise BranchRuntimeFenceError("fence_invalidated_or_unknown")
        ...
        expected_scope = {
            ...
            "worker_slot_id": context.worker_slot_id or context.worker_id,
            "agent_id": context.worker_id or context.worker_slot_id or context.agent_id,
            "allocation_owner": (
                context.allocation_owner
                or context.worker_id
                or context.worker_slot_id
        ...
        supplied_scope = {
            ...
            "actual_host_worker_id": str(actual_host_worker_id or "").strip(),
            ...
            "session_token_ref": presented_ref,
        }
        if (
    ...
    if safe_ref_mode:
        ...
        host_envelope = {
            "schema_version": "mf_subagent_safe_ref_reissue_host_envelope.v1",
            "project_id": saved.project_id,
            "runtime_context_id": runtime_id,
            "contract_execution_id": str(contract_execution_id or "").strip(),
            "task_id": saved.task_id,
            ...
            "session_token_ref": runtime_context_session_token_ref(saved),
            "session_token_ref_source": "safe_ref_prestartup_reissue",
...
def list_active_integration_epochs(
    conn: sqlite3.Connection,
    project_id: str,
) -> list[IntegrationEpoch]:
    """Return every live epoch so cross-target position guards cannot miss one."""
...
def _standalone_integration_epoch_coordination_backlog_id(
    ...
    *,
    ...
    durable_item = _standalone_integration_epoch_durable_item(
        conn,
        item=item,
        batch_id=batch_id,
    )
    return durable_item.backlog_id if durable_item is not None else ""


def _standalone_integration_epoch_durable_item(
    conn: sqlite3.Connection,
    *,
    item: MergeQueueItem,
    batch_id: str,
) -> MergeQueueItem | None:
    """Return the one exact durable row authorized by a standalone batch id."""
    ...
    if (
        ...
        or durable_item.branch_ref != item.branch_ref
        or durable_item.queue_index != item.queue_index
...
def open_or_validate_integration_epoch(
    ...
    *,
    ...
    merged_prefix_items: list[str] = []
    remaining_items: list[str] = []
    first_unmerged_item_id = ""
    non_contiguous_merged_item_id = ""
    for candidate in ordered:
        if candidate.status == STATE_MERGED:
            if first_unmerged_item_id:
                non_contiguous_merged_item_id = candidate.queue_item_id
                break
            merged_prefix_items.append(candidate.queue_item_id)
            continue
        if not first_unmerged_item_id:
            first_unmerged_item_id = candidate.queue_item_id
        remaining_items.append(candidate.queue_item_id)
    if non_contiguous_merged_item_id:
        provisional = IntegrationEpoch(
            ...
            merge_queue_id=item.merge_queue_id,
            merge_cursor=len(merged_prefix_items),
            merged_prefix=tuple(merged_prefix_items),
            remaining_queue_item_ids=tuple(remaining_items),
            active_queue_item_id=first_unmerged_item_id,
            failure_reason="non_contiguous_merged_prefix",
        )
        raise IntegrationEpochFrozenError(
            (
                "batch merge queue has a merged row after the first unmerged "
                "row; repair the durable queue ordering/status drift before "
                "opening an integration epoch"
            ),
            provisional,
        )
    ...
    if not coordination_backlog_id:
        ...
        raise IntegrationEpochFrozenError(
            (
                ...
                "integration epoch; recover or replay the canonical "
                "mf_batch_parallel.entered timeline event for this batch and "
                "merge queue, then retry the same queue position"
            ),
            provisional,
        )
...
def _with_release_binding_audit(
    ...
    *,
    ...
    for record in records:
        ...
        existing_binding_audit = (
            dict(next_record.get("binding_audit") or {})
            if isinstance(next_record.get("binding_audit"), Mapping)
            else {}
        )
    ...
    if canonical_rollover_audits is not None and matching_record_count != 1:
        raise IntegrationEpochUnlandableChildReleaseError(
            ...
            (
                "release replay authority rollover requires exactly one "
                "durable released-child audit projection"
            ),
            queue_item_id=queue_item_id,
            released_child_record_count=matching_record_count,
            zero_write_rejection=True,
        )
    if changed:
...
_RELEASE_OBSERVER_ROUTE_REF_PREFIX = "observer-route-token-ref:"
...
def _release_replay_authority_ref_projection(
    ...
    prefixes = {
        "observer_session_ref": _RELEASE_OBSERVER_SESSION_REF_PREFIX,
        "observer_route_token_ref": _RELEASE_OBSERVER_ROUTE_REF_PREFIX,
        "governing_backlog_ref": _RELEASE_GOVERNING_BACKLOG_REF_PREFIX,
        "governing_task_ref": _RELEASE_GOVERNING_TASK_REF_PREFIX,
        "authorized_action_ref": _RELEASE_AUTHORIZED_ACTION_REF_PREFIX,
    }
    projected = {
        key: tuple(ref for ref in refs if ref.startswith(prefix))
        for key, prefix in prefixes.items()
    }
    projected["stable_refs"] = tuple(
        ref
        for ref in refs
        if not ref.startswith(_RELEASE_OBSERVER_SESSION_REF_PREFIX)
        and not ref.startswith(_RELEASE_OBSERVER_ROUTE_REF_PREFIX)
    )
    return projected


def _validated_release_replay_authority_rollover_audit(
    *,
    project_id: str,
    event_id: int,
    event_operator_principal: str,
    event_refs: Sequence[str],
    current_operator_principal: str,
    current_refs: Sequence[str],
    rollover_proof: Mapping[str, Any] | None,
    now_iso: str,
) -> dict[str, Any]:
    """Validate a server-authorized observer route identity rollover."""

    def reject(reason: str, **details: Any) -> None:
        raise IntegrationEpochUnlandableChildReleaseError(
            "integration_epoch_release_replay_identity_mismatch",
            "release replay authority rollover did not match the immutable audit event",
            event_id=event_id,
            rollover_rejection=reason,
            zero_write_rejection=True,
            **details,
    ...
    proof = dict(rollover_proof or {})
    if (
        str(proof.get("schema_version") or "")
        != "integration_epoch.release_replay_rollover_proof.v1"
        or proof.get("required_action_membership_validated") is not True
        or proof.get("raw_credentials_included") is not False
        or proof.get("server_validated") is not True
        or str(proof.get("authority_mode") or "")
        != "observer_session_route_token_ref"
        or str(proof.get("project_id") or "") != project_id
        or str(proof.get("operator_principal") or "")
        != current_operator_principal
    ):
        reject("server_validated_route_rollover_proof_required")

    old_projection = _release_replay_authority_ref_projection(event_refs)
    new_projection = _release_replay_authority_ref_projection(current_refs)
    for projection_name, projection in (
        ("immutable_event", old_projection),
        ("current_request", new_projection),
    ):
        invalid_markers = {
            key: len(projection[key])
            for key in (
            ...
            if len(projection[key]) != 1
        }
        if invalid_markers:
            reject(
                "release_authority_markers_not_unique",
                projection=projection_name,
                marker_counts=invalid_markers,
            )
        empty_markers = [
            key
            for key, prefix in (
            ...
            if projection[key][0] == prefix
        ]
        if empty_markers:
            reject(
                "release_authority_markers_empty",
                projection=projection_name,
                empty_markers=empty_markers,
            )

    old_session_ref = old_projection["observer_session_ref"][0]
    new_session_ref = new_projection["observer_session_ref"][0]
    if (
        not event_operator_principal.startswith(
            _RELEASE_OBSERVER_SESSION_REF_PREFIX
        )
        or event_operator_principal != old_session_ref
        or not current_operator_principal.startswith(
            _RELEASE_OBSERVER_SESSION_REF_PREFIX
        )
        or current_operator_principal != new_session_ref
    ):
        reject("observer_session_principal_binding_invalid")
    if old_projection["stable_refs"] != new_projection["stable_refs"]:
        reject("non_ephemeral_evidence_refs_changed")

    proof_marker_values = {
        "observer_session_ref": str(proof.get("observer_session_ref") or ""),
        "observer_route_token_ref": str(
            proof.get("observer_route_token_ref") or ""
        ),
        "governing_backlog_ref": str(
            proof.get("governing_backlog_ref") or ""
        ),
        "governing_task_ref": str(proof.get("governing_task_ref") or ""),
        "authorized_action_ref": str(
            proof.get("authorized_action_ref") or ""
        ),
    }
    for key, expected in proof_marker_values.items():
...
def release_integration_epoch_unlandable_child(
    ...
    *,
    ...
    replay_authority_rollover: Mapping[str, Any] | None = None,
    now_iso: str = "",
) -> dict[str, Any]:
    ...
    if existing_event is not None:
        event_refs = _parse_json_array(existing_event["evidence_refs_json"])
        business_replay = (
...
def _standalone_advanced_head_catchup_claimed(
    conn: sqlite3.Connection,
    *,
    item: MergeQueueItem,
    context: BranchTaskRuntimeContext | None,
    ...
    """Return True when durable standalone state claims a non-live checkpoint."""

    batch = str(batch_id or "").strip()
    target = str(target_commit or "").strip()
    if (
        not batch.startswith("standalone-")
        or batch == "standalone-"
        or not target
    ):
        return False
    durable_rows = list_merge_queue_items(
        conn,
        item.project_id,
        item.merge_queue_id,
    )
    ...
    checkpoint_refs = {
        ...
        str(context.target_head_commit or "").strip() if context is not None else "",
    }
    checkpoint_refs.discard("")
    if not checkpoint_refs:
        return False
    if len(checkpoint_refs) == 1 and next(iter(checkpoint_refs)) == target:
        return False
    resolved_checkpoints: set[str] = set()
    for checkpoint_ref in checkpoint_refs:
        checkpoint, _error = _git_preview_commit(
            repo_root,
            checkpoint_ref,
            timeout_seconds=timeout_seconds,
        )
        if not checkpoint:
            return True
        resolved_checkpoints.add(checkpoint)
...
def _derive_standalone_historical_checkpoint_authority(
    ...
    *,
    ...
    if not _git_preview_branch_is_ancestor(
        ...
        return reject("standalone_candidate_not_in_checkpoint")
    if not _git_preview_branch_is_ancestor(
        repo_root,
    ...
    parents = _git_preview_command(
        repo_root,
        ["rev-list", "--parents", "-n", "1", checkpoint],
        timeout_seconds=timeout_seconds,
    )
...
def _apply_standalone_historical_checkpoint_catchup(
    conn: sqlite3.Connection,
    *,
    item: MergeQueueItem,
    context: BranchTaskRuntimeContext,
    ...
    try:
        ...
        if existing_epoch is not None and existing_epoch.status == (
            INTEGRATION_EPOCH_CLOSED
        ):
            idempotent_replay = True
        elif existing_epoch is not None:
            if (
                durable.status != STATE_MERGED
                or durable.queue_item_id not in existing_epoch.merged_prefix
                or not _commit_ref_unambiguously_matches(
                    existing_epoch.current_head,
                    checkpoint,
                )
            ):
                raise RuntimeError(
                    "standalone_historical_checkpoint_epoch_conflict"
                )
            idempotent_replay = True
            ...
            epoch = open_or_validate_integration_epoch(
                ...
                item=replace(
                    ...
                    target_head_before_merge=target_before,
                    target_head_after_merge=checkpoint,
                    current_target_head=checkpoint,
                ),
                batch_id=batch_id,
        ...
        exact_terminal = bool(
            saved_item is not None
            and saved_item.status == STATE_MERGED
            and _commit_ref_unambiguously_matches(
                saved_item.merge_commit,
                checkpoint,
            )
            and _commit_ref_unambiguously_matches(
                saved_item.target_head_after_merge,
                checkpoint,
            )
            and saved_item.snapshot_id == snapshot_id
            and saved_context is not None
            and _commit_ref_unambiguously_matches(
                saved_context.target_head_commit,
                checkpoint,
            )
            ...
            and saved_epoch.snapshot_id == snapshot_id
        )
        if not exact_terminal:
            raise RuntimeError(
                "standalone_historical_checkpoint_attachment_mismatch"
            )
        if (
            auto_record is not None
            and auto_record.get("integration_epoch_barrier")
            != "satisfied_and_closed"
        ):
            raise RuntimeError(
                "standalone_historical_checkpoint_barrier_unsatisfied"
            )
        ...
        return {
            ...
            "integration_epoch": integration_epoch_to_dict(saved_epoch),
            "merge_queue_graph_epoch_auto_record": auto_record,
        }
    except Exception as exc:
        ...
        return {
            "ok": False,
            "error": "standalone_historical_checkpoint_attachment_failed",
    ...
    *,
    ...
    if already_integrated:
        ...
        integrated_item = replace(
            item,
            status=STATE_MERGED,
            merge_commit=merge_position or item.merge_commit,
            target_head_before_merge=(
                merge_position_before
                or item.target_head_before_merge
                or item.current_target_head
                or item.validated_target_head
                or merge_position
            ),
            target_head_after_merge=(
                merge_position or item.target_head_after_merge
            ),
            current_target_head=merge_position or item.current_target_head,
        )
        integrated_items = [
        ...
        if historical_checkpoint:
            applied = _apply_standalone_historical_checkpoint_catchup(
                conn,
                item=item,
                context=item_context,
                batch_id=item_batch_id,
            ...
            if not applied.get("ok"):
                return {
                    ...
                    "message": (
                        "historical standalone checkpoint catch-up rolled back "
            ...
            return {
                ...
                "executed": False,
                "already_integrated": True,
                "idempotent_epoch_replay": bool(
                    applied.get("idempotent_replay")
                ),
                "historical_checkpoint_catchup": True,
                "target_ref_mutated": False,
                "merge_commit": historical_checkpoint,
                "candidate_preview_root": str(repo_root),
                "preview": preview,
                "gate_plan": merge_gate_plan_to_dict(gate_plan),
        ...
        if integration_epoch_required:
            try:
                epoch = open_or_validate_integration_epoch(
                    ...
                    batch_id=item_batch_id,
                    target_head=merge_position or branch_commit,
        ...
        recorded = record_merge_queue_result(
            ...
            status=STATE_MERGED,
            merge_commit=merge_position or branch_commit,
            target_head_before_merge=integrated_item.target_head_before_merge,
            target_head_after_merge=merge_position or branch_commit,

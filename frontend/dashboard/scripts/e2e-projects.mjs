...
function verifyBacklogEvidenceContract() {
  ...
  assert(apiSource.includes("requirePublicSafeTypedDag") && apiSource.includes("response.dag?.typed_edges !== true"), "The dashboard API boundary should reject visualization payloads that are not public-safe, read-only typed DAGs");
...
function verifyTypedDagContract() {
  ...
  const viewSource = readFileSync(path.join(REPO_ROOT, "frontend/dashboard/src/views/BacklogView.tsx"), "utf8");
  const typeSource = readFileSync(path.join(REPO_ROOT, "frontend/dashboard/src/types.ts"), "utf8");
  assert(apiSource.includes("requirePublicSafeTypedDag") && apiSource.includes("response.dag?.typed_edges !== true"), "API boundary should require a public-safe read-only typed DAG");

...
function verifyBacklogEvidenceContract() {
  ...
  assert(taskTimelineSource.includes("stable *current-stream* events") && taskTimelineSource.includes('"synthetic_current": True') && taskTimelineSource.includes('"append_only_history": False'), "Backend compact-ledger projection should synthesize stable Current stream events while declaring them outside append-only history");
  ...
  assert(playbackSource.includes('owner_role: sanitizeTaskPlaybackDagText') && playbackSource.includes('lane_id: sanitizeTaskPlaybackDagText'), "TaskPlayback DAG normalization should retain public role-bound verification and lane identity");
  assert(viewSource.includes("canonicalTimelineLaneForNode") && viewSource.includes("task_playback.canonical_dag_node.v1") && viewSource.includes("No authoritative QA or independent-verification evidence is recorded."), "Backlog Verification should project explicit canonical QA nodes and render an honest empty state without synthetic PASS");
  assert(backlogViewTestSource.includes("buildBacklogCanonicalVerificationFixtureDagForTest") && backlogViewTestSource.includes("buildBacklogEmptyVerificationFixtureDagForTest"), "Backlog fixtures should cover canonical Verification items and the no-evidence empty state");
  assert(viewSource.includes('className="backlog-dag-phase-track"') && viewSource.includes("title={phase}") && viewSource.includes("aria-label={phase}") && cssSource.includes(".backlog-dag-phase-track,\n.backlog-dag-lane-track") && cssSource.includes("--backlog-dag-track-min-width") && cssSource.includes("text-overflow: ellipsis"), "Timeline phase headers should share lane-track geometry and expose accessible non-overlapping labels at modal widths");
  ...
  assert(viewSource.includes("Timeline DAG"), "Backlog detail should keep timeline evidence in the compact detail modal");
  ...
  assert(treeSource.includes('label="Activity"'), "Left navigation should expose Activity as the primary event destination");
  assert(!treeSource.includes('label="Runtime event"') && !treeSource.includes('label="Runtime events"'), "Left navigation should not expose singular Runtime event copy");
  ...
  assert(playbackSource.includes("contractRuntimeCompatibilityRepairValues") && playbackSource.includes("source_missing_repair_id"), "Shared playback semantics should format typed compatibility repairs and explicit missing-id fallbacks");
  assert(playbackViewSource.includes("Compatibility repair target (advisory)") && playbackViewSource.includes('surface="Playback"') && playbackViewSource.includes("compatibility repair targets (advisory)"), "Activity Current and Playback should share advisory compatibility repair labels");
  assert(viewSource.includes("Compatibility repair targets (advisory)") && viewSource.includes("contractRuntimeCompatibilityRepairValues"), "Backlog detail should reuse the shared compatibility repair formatter");
  assert(playbackSource.includes("authoritative_next_actions") && playbackSource.includes("taskPlaybackNextLegalActionPresentations"), "Playback semantics should preserve every ContractRuntime/current-chain next_legal_action for presentation");
  assert(playbackViewSource.includes("function NextLegalActionCallout") && playbackViewSource.includes("selectedFrameId={activityTrace.frames[activityTrace.frames.length - 1]?.id") && playbackViewSource.includes('surface="Current"') && playbackViewSource.includes('<NextLegalActionCallout trace={activeTrace} selectedFrameId={activeFrameId} surface="Playback"'), "Activity Current and Playback must reuse the same dedicated next-legal-action callout");
  assert(playbackViewSource.includes("data-next-legal-action-callout") && playbackViewSource.includes("data-next-legal-action-authority") && playbackViewSource.includes("data-next-legal-action-disposition"), "Next legal actions should use semantic, visually dedicated callout markup rather than metadata chips");
  assert(playbackViewSource.includes("ContractRuntime/current-chain first · historical actions advisory") && playbackViewSource.includes('item.disposition === "BYPASSED"') && playbackViewSource.includes('item.disposition === "WAIVED"'), "The shared callout must keep legacy/history advisory and bypass/waiver visibly non-PASS");
  assert(playbackTestSource.includes("every ContractRuntime/current-chain next_legal_action") && playbackTestSource.includes("Selected event next legal action (advisory)") && playbackTestSource.includes("Legacy next legal action (advisory)"), "Deterministic fixtures should cover Current/Playback canonical and advisory next-action shapes");
  assert(playbackTestSource.includes('status: "blocked"') && playbackTestSource.includes('status: "bypassed"') && playbackTestSource.includes('status: "waived"'), "Deterministic next-action fixtures should cover blocked, bypassed, and waived states");
  assert(playbackTestSource.includes("event #15441 should expose typed repair target") && playbackTestSource.includes("must not produce a red/unmet repair target"), "Compatibility fixtures should cover event #15441 and blocked=false/0/empty semantics");
  assert(playbackTestSource.includes("Compatibility source: timeline:15442") && playbackTestSource.includes("payload.reason: legacy gate reported a block without a stable identifier"), "Compatibility fixtures should cover labeled sanitized raw fallback identity");
  ...
  assert(
    playbackPanelSource.includes("Latest")
      && playbackPanelSource.includes("Current")
  ...
  assert(
    playbackPanelSource.includes("route_alerts")
      ...
      && playbackPanelSource.includes("blocked_actions")
      && playbackPanelSource.includes("required_lanes_evidence")
      && playbackPanelSource.includes("visible_injection_manifest_hash"),
    "Task playback read-receipt inspector should extract route alerts, action bounds, required evidence, and visible injection refs",
  );
  ...
  assert(playbackTestSource.includes("event #1750 summary should explain backlog, route identity, prompt contract scope, and evidence counts"), "Task playback fixture should cover event #1750 structured summary");
  assert(playbackSource.includes("closed_rows") && playbackSource.includes("implemented_and_merged") && playbackSource.includes("remaining_acceptance") && playbackSource.includes("remaining_open"), "Task playback normalizer should promote observer audit remaining-scope payload facts");
  assert(playbackTestSource.includes("event #329 summary should mention the audit decision, remaining scope, and next legal action"), "Task playback fixture should cover event #329 observer audit remaining-scope summary");
  ...
  assert(playbackTestSource.includes("body_persisted_status") && playbackTestSource.includes("raw launch text not persisted"), "Task playback fixture should cover read-receipt body persistence status");
  assert(playbackTestSource.includes("route_alerts") && playbackTestSource.includes("allowed_actions") && playbackTestSource.includes("blocked_actions"), "Task playback fixture should cover route alerts and action bounds");

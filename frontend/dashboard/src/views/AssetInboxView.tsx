    ...
    <div className="view asset-browser-view">
      ...
      <section className="asset-relation-browser">
        ...
        <aside className="asset-relations-panel">
          ...
          {selectedItem ? (
            <RelationPanel item={selectedItem} relations={selectedRelations} />
          ) : (
...
function RelationPanel({ item, relations }: { item: AssetInboxItem; relations: RelationView[] }) {
  const accepted = relations.filter((relation) => relation.status === "accepted");
  const candidates = relations.filter((relation) => relation.status !== "accepted");
  if (relations.length === 0) {
    return (
      <div className="asset-browser-empty">
        No accepted or candidate graph relations for <span className="mono">{item.path}</span>.
      </div>
    );
  }
  return (
    <div className="asset-relation-stack scrollbar-thin">
      <RelationGroup title="Accepted" relations={accepted} empty="No accepted binding." />
      <RelationGroup title="Candidates" relations={candidates} empty="No candidate proposal." />
    </div>
  );
}

function RelationGroup({ title, relations, empty }: { title: string; relations: RelationView[]; empty: string }) {
  return (
    <section className="asset-relation-group">
      <div className="asset-relation-group-title">
        <span>{title}</span>
        <span className="asset-chip-count">{relations.length}</span>
      </div>
      {relations.length === 0 ? (
        <div className="asset-browser-muted">{empty}</div>
      ) : (
        relations.map((relation) => (
          <div key={relation.relation_id} className={`asset-relation-card ${relation.status}`}>
            <div className="asset-relation-card-head">
              <span className="mono">{relation.target_node_id}</span>
              <span className={`asset-relation-state ${relation.status}`}>{relation.status}</span>
            </div>
  ...
  return (
    <DetailBlock title="Governance hint">
      ...
      <div className={`attach-state attach-state-${props.result.state}`}>
        {!hintable
          ? "Direct hint binding is available for unbound doc/test/config assets."
          : supported
            ? props.result.message
            : "Direct write unsupported for this file type."}
      </div>
    </DetailBlock>
  );
}
...
function isHintable(item: AssetInboxItem): boolean {
  ...
  return (
    ...
    ["doc_unbound", "doc_candidate", "test_candidate", "config_pending_decision"].includes(item.asset_status)
  );
}

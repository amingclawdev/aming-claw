...
| R19 | Instantiated contract gate | Parallel MF work, subagent work, dashboard/API/user-visible behavior, or any task with explicit evidence requirements | Observer MUST instantiate a source-controlled contract template such as `agent/governance/contract_templates/mf_parallel.v1.json` into `chain_trigger_json.parallel_contract` before delegation. Required evidence items MUST have stable `id` values. For `mf_sub` workers, the instance MUST include task metadata `task_id`, `parent_task_id`, `worker_role=mf_sub`, and `fence_token`; before `spawn_agent` or host/Codex worker launch, governed nontrivial, Judge-routed, `mf_parallel.v1`, or parent-route-bound workers MUST have machine-consumable `mf_subagent_branch_runtime.v1` evidence from the MCP `parallel_branch_allocate` wrapper when MCP is available, falling back to `POST /api/graph-governance/{project_id}/parallel-branches/allocate` or `upsert_branch_context` only outside MCP. `registration_ref` / `source_ref` names the allocation source; `runtime_context_id` is the worker contract identity. The evidence must match task, parent task where available, fence, worktree, base commit, target head commit, and merge queue. Marker-only source refs are not dispatch-ready evidence. Only after that registration should the worker run audited graph lookup before implementation with explicit `query_source=mf_subagent` and the same identity context; dispatch/finish gates MUST carry normalized `mf_subagent_graph_trace.v1` evidence and MUST NOT infer query source from trace ids. Observer-to-subagent service dispatch evidence MUST be machine-validatable as `observer_subagent_service_dispatch.v1`: either replayable `dispatch_command_ref` plus `monitor_ref`, or a documented host-adapter boundary with a documentation ref. The observer runtime text service prepares host/Codex worker launch text without invoking a model, ServiceManager, or executor worker; persistent timeline evidence stores only `runtime_context_id` and `launch_text_hash`, never raw launch text. Timeline evidence MUST record returned graph trace ids through `payload.graph_trace_ids`, `payload.graph_query_trace_ids`, `payload.graph_trace_evidence.trace_ids`, `verification.graph_trace_ids`, `verification.graph_query_trace_ids`, `verification.graph_trace_evidence.trace_ids`, or `verification.contract_evidence[].graph_trace_ids`. Timeline evidence MUST reference requirement ids through `payload.requirement_id(s)`, `verification.requirement_id(s)`, or `verification.contract_evidence[].requirement_id`. `handle_backlog_close` blocks MF close until every required contract evidence id is present with a passing status. |
...
from MCP `parallel_branch_allocate`,
`POST /api/graph-governance/{project_id}/parallel-branches/allocate`, or
`upsert_branch_context`. In CLI, MCP, and API inputs,
`branch_runtime_registration_ref` / `registration_ref` / `source_ref` is the
allocation source/API/CLI reference; it is not the worker identity.
`runtime_context_id` is the worker contract identity and must be carried
separately. On the server path, prepare may resolve a supplied
`runtime_context_id` through the persisted parallel branch runtime context, but
it must still return dispatch-ready only when task, parent/root task, fence,
worktree, base commit, target head commit, and merge queue match the allocation
row. A marker-only registration ref, or a bare `runtime_context_id` without a
matching persisted allocation row, MUST return `status=allocation_required`,
...
#### R19.2 Local `mf_sub` Dispatch Gate

Before `spawn_agent`, the observer MUST allocate/register branch runtime context
through MCP `parallel_branch_allocate` when the wrapper is available. Outside
MCP, fall back to
`POST /api/graph-governance/{project_id}/parallel-branches/allocate` or
`agent.governance.parallel_branch_runtime.upsert_branch_context`. Then run and
record `agent.governance.mf_subagent_contract.validate_mf_subagent_dispatch_gate`
for each local `mf_sub` worker. The gate must pass before non-blocking dispatch
and must prove an isolated branch/worktree/file fence, `base_commit`,
`target_head_commit`, `fence_token`, owned files, dirty-scope evidence, and
matching `mf_subagent_branch_runtime.v1` registration evidence for governed
nontrivial, Judge-routed, or parent-route-bound workers. Without this
...
Allocation evidence must keep `registration_ref` / `source_ref` distinct from
`runtime_context_id`: the former names the allocation source, the latter is the
worker contract identity. Workers may poll the runtime contract by
`runtime_context_id` plus

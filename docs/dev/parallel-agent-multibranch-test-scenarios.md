...
| PB-007 | Chain compatibility | A Chain stage records branch runtime identity while MF remains the active client. | Accept optional `chain_id`, `root_task_id`, `stage_task_id`, `stage_type`, and `retry_round`; runtime semantics do not require Chain execution. | Implemented for runtime identity; full Chain adapter execution remains future work. | `test_parallel_branch_runtime.py` |
